    ...
    def insert(self, board):
        ...
        for i in range(3):
            ...
            if (self.__start[1] + i) > columns - columnsAtATime:
                # if not board.grid[self.__start[0]][self.__start[1] + i].isEnemy:
                board.grid[self.__start[0]][self.__start[1] + i].display = Back.BLACK + \
                    Fore.BLUE + Style.BRIGHT + self.display[i] + Style.RESET_ALL

                # elif board.grid[self.__start[0]][self.__start[1] + i].display == (Back.BLACK + Fore.BLUE + Style.BRIGHT + ' ' + Style.RESET_ALL):
                #     board.grid[self.__start[0]][self.__start[1] + i].display = Back.BLACK + Fore.BLUE + Style.BRIGHT + self.display[i] + Style.RESET_ALL
